...
## Why this is different · 这套东西在解决什么

这个项目的全部设计，都来自一个很硬的约束：

> **我的 Agent 执行环境是「每轮启动、轮末结束」的** —— 每收到一条消息就 spawn 一个进程，
> 回复完整个进程组就被杀。它没有定时器、没有后台、也没法承诺"我等会儿回来看"。

在这个前提下，要让「耗时的活继续跑，并把结果准确送回原来那条话题」，就必须自己造三样东西。

### ① Fleet — 一条需求，7 路并行出勤

不是在一个进程里开几个线程，而是 **fan-out 出最多 7 个相互独立、跨轮存活的子会话**。
每个子会话托管在常驻 bot 名下、跑在自己的话题里、有自己的工作目录甚至自己的模型；
跑完自动把结果回贴到主话题，**整批收工后主动唤醒主 Agent** 做聚合决策。
主 Agent 派完活可以直接收工——它不需要在那儿干等，而这正是"派完就被杀"的执行环境里最关键的一步。
...
把 **Claude、GPT、Gemini 等异构 Agent** 各接成一个**独立的机器人**，拉进同一个群，让它们互相 @。
一方负责实现，另一方必须**回到 ground truth** 独立复核——
链上解码、查库对账、无头浏览器真跑一遍 UI、curl 探针打真实接口——而不是采信对方"自报的结论"。

让同一个模型自查，它会带着自己那套系统性盲区一起批卷子；
换一个**厂商、训练数据、失败模式都不同**的模型来挑刺，才更容易逮住幻觉出的表名、想当然的字段和没跑过的分支。
两边都签字，才算交付。

### ③ Runtime MCP — 让工作跨过轮次边界

前面那个约束的直接后果是：Agent 说"我 10 分钟后回来查 CI"，没有任何人会兑现它。
cc-lark 自研的 `cc_mcp_server.py` 把 13 个运行时工具注入进去，**兑现方是常驻的 bot 进程**：
