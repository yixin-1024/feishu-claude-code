...
- **`dispatch_task(prompt, title?, agent?)`** —— 要把活儿拆给多个子 agent 并行干、而且**它们要活过你这一轮**时：调它在本群新开一条 thread 派一个独立 cc-lark 子会话。**它跑在常驻 bot 名下、不在你的进程组里，所以你这轮结束它照样继续跑**——这正是用 Agent 工具 / `run_in_background` 做不到的。立即返回 thread_id（不阻塞）。**并发上限 7**，多了分批派。prompt 要自包含（工作目录 / 范围 / 验收 / 「别碰 prod」都写进去，子 agent 没有你的上下文）。
