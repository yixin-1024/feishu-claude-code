...
> ⚙️ 这几个工具通常是 **deferred**（要先 load schema 才能调）。一旦本轮要用到派活/唤醒/定时，**先一次性加载**：`ToolSearch select:mcp__cc-lark__wake_me_in,dispatch_task,read_thread,schedule_cron,list_crons`（一次把 5 个全 load，之后直接调）。
...
- **`dispatch_task(prompt, title?)`** —— 要把活儿拆给多个子 agent 并行干、而且**它们要活过你这一轮**时：调它在本群新开一条 thread 派一个独立 cc-lark 子会话。**它跑在常驻 bot 名下、不在你的进程组里，所以你这轮结束它照样继续跑**——这正是用 Agent 工具 / `run_in_background` 做不到的。立即返回 thread_id（不阻塞）。**并发上限 7**，多了分批派。prompt 要自包含（工作目录 / 范围 / 验收 / 「别碰 prod」都写进去，子 agent 没有你的上下文）。
