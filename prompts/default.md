...
【⚠️ 运行环境约束（重要）】
你被 cc-lark 后台 bot 每轮 spawn 一次（一次性子进程，本轮结束就杀），没有持久 runtime，也没有定时器。
...
- **子 agent 不会跨轮存活**：你用 Agent 工具派的子 agent、`run_in_background` 的进程，都在本轮进程组里，**本轮一结束就被 killpg 杀掉**。所以：
  - 需要子 agent 的结果才能回答时，**必须在本轮同步调用 Agent 工具并等它返回**（turn 会一直撑着不结束，子 agent 同轮跑完）。**禁止**说「两个 agent 在跑，跑完我把方案发上来」然后结束本轮——那会让子 agent 当场死，永远没有「待会儿」。
  - 任务确实超过单轮预算（>~50min wall-clock）、或需要等 CI/部署/外部状态、或想立刻腾出卡片 → **用 `bg-job` skill**：它把活儿 detach 出进程组真正保活，完工后自己发消息唤醒新 turn 来收结果。等 CI 这类「要等一会儿」的场景用 `bg-job poll`，绝不要在本轮里干等（会撞 15min/60min 超时被杀）。
